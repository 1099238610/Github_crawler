...
class DatetimePipeline:
    ...
    def process_item(self, item, spider):
        for i in range(len(item['issue_list'])):
            if item['issue_list'][i]['datetime']:
                datetime = item['issue_list'][i]['datetime']
                # 如果日期不为空
                if datetime is not None:
                    if len(datetime) > 1:
                        item['issue_list'][i]['datetime'] = datetime[1]
                    else:
                        item['issue_list'][i]['datetime'] = datetime[0]
        return item
